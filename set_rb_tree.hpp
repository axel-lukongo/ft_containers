...
struct set_node{
	...
	set_node<T> *left, *right;
	set_node<T> *_parent;


	public:
		set_node(T key):_key(key){
			left = right = _parent = NULL;
...
namespace ft{
	...
	public:
		...
		typedef typename ft::set_rb_iterator<Node>						iterator;
		typedef typename ft::const_set_rb_iterator<Node>				const_iterator;
	...
	public:
		...
		/*                          operator                        */
		/************************************************************/
		set_rb_tree& operator=(set_rb_tree& x){
			_count = x._count;
			_root = x._root;
			_alloc = x._alloc;
		...
		ft::pair<iterator, bool> add_one(const value_type &val){
			if (_root == NULL){ //if my tree is empty
				...
				node_ptr  _node = _alloc.allocate(1);
								_alloc.construct(_node,  Node(val));
				node_ptr ptr = find_node(_node);
				if(_root->_key == _node->_key || ptr != NULL){ // if this key doesn't exist yet
		...
		ft::pair<iterator, bool> add_one(iterator position, const value_type &val){
			if (_root == NULL){ //if my tree is empty
				...
				if (position.base()){
					...
					while (!_cmp(_node->_key, ptr->_key) && ptr->_parent)
						ptr = ptr->_parent;
					add(ptr, _node);
				}
				else{
					if(_root->_key == _node->_key || find_node(_node) != NULL){
						_alloc_node.destroy(_node);
						_alloc_node.deallocate(_node, 1);
	...
	//*************************** private add *********************************/
			void add(node_ptr _parent, node_ptr new_node){
				...
				if(_cmp(_parent->_key, new_node->_key)){
				...
				else if ( _cmp(new_node->_key, _parent->_key)){
	...
	//*************************** find *********************************/
			iterator	find(const key_type& my_key) {
				node_ptr ptr = _root;
				while (ptr) {
					if (_cmp(ptr->_key, my_key)) // if ptr->_key < my_key, _cmp will return true.
						ptr = ptr->right;
					else if (_cmp(my_key, ptr->_key))
						ptr = ptr->left;
					else
	...
	//*************************** upper_bound *********************************/
			iterator	upper_bound (const key_type& my_key){
				...
				while (ptr != save){
					...
					else if(!_cmp(*ptr, my_key) && !_cmp(my_key, *ptr))
						return ++ptr;
					else
						return ptr;
				}
	...
	//***************************** erase **********************************/
			...
			void remove_node(node_ptr z){
				...
				//both condition it work when we are on aa leef
				...
				else { //i came here when the node have a childrens on left and right 
					y = _minimum(z->right);
					original_color = y->_black; // save the color
					x = y->right;
			...
			void	_transplant(node_ptr u, node_ptr va) {
					if (!u->_parent) { // if the u parent is NULL (it mean we are on the root)
						_root = va;
					}
					else if (u == u->_parent->left) { // if u is a left child
						u->_parent->left = va;
					}
					else 
						u->_parent->right = va;
			...
			void fixe_delete(node_ptr x){
				while (x && x != _root && x->_black == true) {
							if (x == x->_parent->left) { // this condition is when im on the left
	...
	// private:
		...
		node_ptr find_node(node_ptr &my_key){
			node_ptr ptr = _root;
			if(my_key != NULL){
				while (ptr){
					if (_cmp(my_key->_key, ptr->_key))
						ptr = ptr->left;
					else if (_cmp( ptr->_key, my_key->_key))
						ptr = ptr->right;
					else
						return ptr;
				}
